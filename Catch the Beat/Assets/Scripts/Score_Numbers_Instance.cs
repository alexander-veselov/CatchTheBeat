...
public class Score_Numbers_Instance : MonoBehaviour {
	...
	void Start () {
		...
		animation = Instantiate (Resources.Load<Score_Numbers> ("Prefabs/num_pref"), pos, transform.rotation);

		main = Instantiate (Resources.Load<Score_Numbers> ("Prefabs/num_pref"), pos, transform.rotation);
			
		nums_to_disp = animation.GetComponentsInChildren<SpriteRenderer> ();
